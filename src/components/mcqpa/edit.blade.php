...
@php $que = DB::table('fmt_mcqpa_ques')->where('id', $message)->first(); @endphp
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mcqpa.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    <div class="flex flex-wrap -mx-4 my-2">{{-- flex-wrap --}}
                        <div class="w-1/2 px-4">{{-- w-1/2 --}}
                            <div class="my-2">
                                ...
                                <input type="file" accept="audio/*" name="ques_audio" class="w-full border border-gray-500 rounded-lg p-2">
                            </div>
                        </div>{{-- //w-1/2 --}}
                        <div class="w-1/2 px-4">{{-- w-1/2 --}}
                            <div class="my-2">
                                <label class="bloc" for="">Image</label>
                                <input type="file" accept="image/*" name="ques_image" class="w-full border border-gray-500 rounded-lg p-2">
                            </div>
                        </div>{{-- //w-1/2 --}}
                    </div>{{-- //flex-wrap --}}
                    <div class="my-2">
                    ...
                    @php $answers = DB::table('fmt_mcqpa_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mcqpa_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
